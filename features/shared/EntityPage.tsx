...
export default function EntityPage({ config }: { config: EntityConfig }) {
  ...
  async function save() {
    ...
    } catch (e: any) { setError(e?.message || 'Gagal menyimpan data.') }
  ...
  return <div className="module-page">
    ...
    <section className="module-card">
      <div className={styles.toolbar}><div><strong>Daftar {config.title}</strong><span>{loading ? 'Memuat data…' : `${filteredRows.length} dari ${rows.length} data`}</span></div><div className={styles.actions}><label className={styles.search}><span>⌕</span><input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Cari ${config.title.toLowerCase()}…`} /></label><Button variant="secondary" onClick={load} disabled={loading}>Muat ulang</Button></div></div>
      {loading ? <EmptyState title="Memuat data" text="Mengambil data dari Supabase." /> : filteredRows.length ? <Table columns={config.columns} rows={filteredRows} columnLabels={config.columnLabels} onEdit={!config.readOnly && config.fields?.length ? startEdit : undefined} /> : <EmptyState title={search ? 'Data tidak ditemukan' : 'Belum ada data'} text={search ? 'Coba kata kunci lain.' : 'Belum ada record yang tersedia untuk modul ini.'} />}
    </section>
