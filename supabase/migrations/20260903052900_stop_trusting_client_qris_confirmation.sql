...
begin
  ...
  if fn is null then
    raise exception 'checkout_sale_multi_payment tidak ditemukan';
  end if;

  fn := replace(
    fn,
    'if v_qris_confirmed then raise exception ''Konfirmasi QRIS harus berasal dari proses verifikasi pembayaran''; end if;',
