...
begin
  ...
  if p_idempotency_key is not null then
    select pay.sale_id,s.sale_no,s.total_amount,s.paid_amount,s.change_amount,s.status::text into v_existing_sale,v_existing_sale_no,v_existing_total,v_existing_paid,v_existing_change,v_existing_status
    ...
    if v_existing_sale is not null then
      return query select v_existing_sale,v_existing_sale_no,v_existing_total,v_existing_paid,v_existing_change,v_existing_status;
      return;
    end if;
  end if;
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    v_total:=v_total+((v_item->>'qty')::numeric*v_price)::bigint;
  ...
  values(v_sale_id,v_business_id,p_branch_id,p_location_id,p_customer_id,v_sale_no,'OPEN'::public.document_status,v_total,0,0,0,0,v_total,0,0,v_hpp,v_total-v_hpp,'POS');
  ...
  for v_payment,v_idx in select item,ordinality from jsonb_array_elements(p_payments) with ordinality as t(item,ordinality) loop
    ...
    v_qris_confirmed:=coalesce((v_payment->>'qris_confirmed')::boolean,false);
    ...
    if upper(v_method.method_type)='CASH' or v_method.code='CASH' then
      ...
      -- Client input is never trusted to mark QRIS paid. A provider/webhook flow must verify it later.
      v_qris_confirmed:=false;
