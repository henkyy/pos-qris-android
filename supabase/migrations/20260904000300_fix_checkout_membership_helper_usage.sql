-- current_business_ids/current_branch_ids return SETOF uuid, not named records.
-- Fix the membership checks added by the receivable/checkout hardening migration.
...
declare
  v_business_id uuid;
  v_sale_id uuid := gen_random_uuid(); v_sale_no text := 'AR-' || to_char(clock_timestamp(),'YYMMDDHH24MISSMS');
  v_subtotal bigint := 0; v_discount bigint := greatest(coalesce(p_discount_amount,0),0); v_total bigint := 0; v_hpp bigint := 0;
  v_item jsonb; v_product record; v_price bigint; v_conversion numeric; v_customer_term integer := 0;
  v_existing_sale uuid; v_existing_sale_no text; v_existing_subtotal bigint; v_existing_discount bigint; v_existing_total bigint; v_existing_paid bigint; v_existing_change bigint; v_existing_status text;
begin
  ...
  if not exists (select 1 from private.current_branch_ids() x where x=p_branch_id) then raise exception 'Akses cabang ditolak'; end if;
  ...
  select c.payment_term_days into v_customer_term from public.customers c where c.id=p_customer_id and c.business_id=v_business_id and c.is_active=true;
  if not found then raise exception 'Pelanggan tidak valid'; end if;
  if p_idempotency_key is not null then
    select s.id,s.sale_no,s.subtotal,s.discount_amount,s.total_amount,s.paid_amount,s.change_amount,s.status::text into v_existing_sale,v_existing_sale_no,v_existing_subtotal,v_existing_discount,v_existing_total,v_existing_paid,v_existing_change,v_existing_status
    from public.sales s where s.business_id=v_business_id and s.customer_id=p_customer_id and s.notes='POS_RECEIVABLE:'||p_idempotency_key limit 1;
    if v_existing_sale is not null then return query select v_existing_sale,v_existing_sale_no,v_existing_subtotal,v_existing_discount,v_existing_total,v_existing_paid,v_existing_change,v_existing_status; return; end if;
  end if;
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    v_subtotal:=v_subtotal+((v_item->>'qty')::numeric*v_price)::bigint; v_hpp:=v_hpp+((v_item->>'qty')::numeric*coalesce(v_product.current_cost,0))::bigint;
  end loop;
  ...
  v_total:=v_subtotal-v_discount; if v_total<=0 then raise exception 'Total setelah diskon harus lebih dari 0'; end if;
...
create or replace function public.checkout_sale_multi_payment_v2(p_branch_id uuid,p_location_id uuid,p_customer_id uuid,p_items jsonb,p_payments jsonb,p_discount_amount bigint default 0,p_idempotency_key text default null)
returns table(sale_id uuid,sale_no text,subtotal_amount bigint,discount_amount bigint,total_amount bigint,paid_amount bigint,change_amount bigint,sale_status text)
language plpgsql security invoker set search_path = '' as $$
declare v_business_id uuid; v_method_id uuid; v_method_code text;
begin
  select br.business_id into v_business_id from public.branches br where br.id=p_branch_id and br.is_active=true;
  if v_business_id is null then raise exception 'Cabang tidak valid atau tidak aktif'; end if;
  if not exists (select 1 from private.current_business_ids() x where x=v_business_id) then raise exception 'Akses bisnis ditolak'; end if;
  ...
  if jsonb_array_length(p_payments)=1 then
    ...
    if v_method_code='RECEIVABLE' then return query select * from public.checkout_sale_receivable_v2(p_branch_id,p_location_id,p_customer_id,p_items,p_discount_amount,p_idempotency_key); return; end if;
  end if;
