...
type MenuId = 'Dashboard' | 'Kasir' | 'Penjualan' | 'Pesanan' | 'Pembayaran' | 'Produk' | 'Stok' | 'Pergerakan Stok' | 'Stok Opname' | 'Penyesuaian Stok' | 'Purchase Order' | 'Penerimaan Barang' | 'Hutang' | 'Supplier' | 'Pelanggan' | 'Piutang' | 'Riwayat Transaksi' | 'Laporan Penjualan' | 'Produk Terlaris' | 'Laporan Stok' | 'Laporan Pembelian' | 'Laporan Piutang' | 'Laporan Hutang' | 'Pengeluaran' | 'Laba & Margin' | 'Cabang / Outlet' | 'Karyawan & Akses' | 'Shift Kasir' | 'Metode Pembayaran' | 'Audit Log' | 'Pengaturan' | 'Bisnis' | 'Outlet' | 'Satuan' | 'Kategori' | 'Harga' | 'QRIS' | 'Pengaturan Sistem'
...
const navGroups: NavGroup[] = ['TRANSAKSI','INVENTORI','PEMBELIAN','PELANGGAN','LAPORAN','MANAJEMEN'].map(id => ({ id, label: id, menus: menus.filter(menu => menu.group === id) }))
const mobileMenus = menus.filter(x => ['Dashboard','Kasir','Stok','Laporan Penjualan'].includes(x.id))

const existing: Partial<Record<MenuId, boolean>> = { Dashboard:true, Kasir:true, Penjualan:true, Pesanan:true, Pembayaran:true, Produk:true, Stok:true, Pelanggan:true, Supplier:true, Pembelian:true, Piutang:true, Laporan:true, Pengaturan:true }
