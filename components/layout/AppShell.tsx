...
type MenuId = 'Dashboard' | 'Kasir' | 'Held Sales' | 'Penjualan' | 'Retur Penjualan' | 'Pesanan' | 'Pembayaran' | 'Produk' | 'Stok' | 'Pergerakan Stok' | 'Stok Opname' | 'Penyesuaian Stok' | 'Transfer Stok' | 'Purchase Order' | 'Penerimaan Barang' | 'Hutang' | 'Pembayaran Hutang' | 'Supplier' | 'Pelanggan' | 'Piutang' | 'Pembayaran Piutang' | 'Riwayat Transaksi' | 'Laporan Penjualan' | 'Produk Terlaris' | 'Laporan Stok' | 'Laporan Pembelian' | 'Laporan Piutang' | 'Laporan Hutang' | 'Pengeluaran' | 'Laba & Margin' | 'Cabang / Outlet' | 'Karyawan & Akses' | 'Shift Kasir' | 'Metode Pembayaran' | 'Audit Log' | 'Kas' | 'Bisnis' | 'Outlet' | 'Satuan' | 'Kategori' | 'Harga' | 'QRIS' | 'Pengaturan Sistem'
type ViewMode = 'retail' | 'distributor' | 'fnb'
type Branch = Record<string, any>
type GroupId = 'TRANSAKSI' | 'INVENTORI' | 'PEMBELIAN' | 'PELANGGAN' | 'KEUANGAN' | 'LAPORAN' | 'MANAJEMEN' | 'PENGATURAN'
...
const menuItems: Menu[] = [
  ...
  { id: 'Transfer Stok', label: 'Transfer Stok', icon: '⇄', group: 'INVENTORI', description: 'Pindahkan stok antar lokasi' },
  ...
  { id: 'Hutang', label: 'Hutang', icon: 'Rp', group: 'PEMBELIAN', description: 'Hutang supplier dan status jatuh tempo' },
  { id: 'Pembayaran Hutang', label: 'Pembayaran Hutang', icon: '↓', group: 'PEMBELIAN', description: 'Catat pembayaran hutang supplier' },
  ...
  { id: 'Pembayaran Piutang', label: 'Pembayaran Piutang', icon: '↑', group: 'PELANGGAN', description: 'Catat pembayaran tagihan pelanggan' },
  { id: 'Riwayat Transaksi', label: 'Riwayat Transaksi', icon: '▤', group: 'PELANGGAN', description: 'Telusuri transaksi pelanggan' },
  { id: 'Shift Kasir', label: 'Shift Kasir', icon: '◷', group: 'KEUANGAN', description: 'Kelola shift dan rekonsiliasi kasir' },
  { id: 'Kas', label: 'Kas', icon: 'Rp', group: 'KEUANGAN', description: 'Pantau pergerakan kas operasional' },
  { id: 'Pengeluaran', label: 'Pengeluaran', icon: '−', group: 'KEUANGAN', description: 'Catat dan pantau biaya operasional' },
...
function renderFeature(active: MenuId, mode: ViewMode, version: number) {
  ...
  switch (active) {
    ...
    case 'Pembayaran Piutang': return <OperationsPage key={key} menu="Pembayaran Piutang" />
    case 'Pembayaran Hutang': return <OperationsPage key={key} menu="Pembayaran Hutang" />
    case 'Transfer Stok': return <OperationsPage key={key} menu="Transfer Stok" />
    case 'Kas': return <OperationsPage key={key} menu="Kas" />
...
export default function AppShell() {
  ...
  const [open, setOpen] = useState<Record<GroupId, boolean>>({ TRANSAKSI: true, INVENTORI: true, PEMBELIAN: false, PELANGGAN: false, KEUANGAN: false, LAPORAN: false, MANAJEMEN: false, PENGATURAN: false })
  ...
  useEffect(() => {
    try {
      ...
      if (savedGroups) { const parsed = JSON.parse(savedGroups); if (parsed && typeof parsed === 'object') setOpen(current => ({ ...current, ...parsed })) }
    } catch {}
    let cancelled = false
    void (async () => {
      try {
  ...
  useEffect(() => { const item = menuItems.find(menu => menu.id === active); if (item) setOpen(current => current[item.group] ? current : { ...current, [item.group]: true }) }, [active])
  ...
  const selectBranch = (id: string) => { const branch = branches.find(item => item.id === id); if (!branch || branch.id === branchId) return; setStoredBranchId(branch.id); setBranchId(branch.id); setVersion(current => current + 1); window.dispatchEvent(new CustomEvent('qris-workspace-changed', { detail: { branchId: branch.id } })) }
  ...
  return <div className={`${styles.shell} ${collapsed ? styles.sidebarIsCollapsed : ''}`}>
    <aside className={styles.sidebar}>
      <div className={styles.brand}>
        ...
        <div className={styles.brandCopy}><div className={styles.brandTitle}>QRIS POS</div><div className={styles.brandSub}>{businessName || 'Toko / Business'}</div></div>
      ...
      <div className={styles.workspaceCard}>
        ...
        <div className={styles.workspaceCopy}><span>OUTLET AKTIF</span><strong>{activeBranchName}</strong></div>
